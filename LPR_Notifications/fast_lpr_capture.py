...
class FastLPRCapture:
    ...
    async def capture_plates(self):
        ...
        try:
            ...
            # On first run, use time of newest event in database
            if self.last_check is None:
                newest = self.lpr_table.find_one(sort=[('timestamp', -1)])
                if newest:
                    # Convert to timezone-aware UTC datetime to match event.start
                    self.last_check = newest['timestamp'].replace(tzinfo=None)
                    logger.info(f"Starting from last captured event: {self.last_check}")
                else:
                    # No previous events, start from 24 hours ago
                    self.last_check = datetime.utcnow() - timedelta(days=1)
                    logger.info(f"No previous events, starting from 24 hours ago")
            
            start = self.last_check
            
            # Get all events (Note: start/limit params don't work in this API version)
            # Fetch all events and filter locally by time
            events = await self.protect.get_events()
            
            # Filter for LPR camera events with license plate detections
            for event in events:
                # Remove timezone info from event.start to compare with naive datetime from DB
                event_time = event.start.replace(tzinfo=None) if hasattr(event.start, 'replace') else event.start
                
                # Only process events since last check
                if event_time <= start:
                    continue
                ...
                if event.metadata and event.metadata.detected_thumbnails:
                    for thumb in event.metadata.detected_thumbnails:
                        ...
                        if thumb.type == 'vehicle' and thumb.name:
                            ...
                            if len(name_str) > 0 and name_str != 'NONE':
                                license_plate = name_str
                                # Convert confidence - it's already 0-100 scale from UniFi
                                confidence = int(float(thumb.confidence)) if thumb.confidence else 0
                ...
                self.lpr_table.insert_one(doc)
                self.stats['stored'] += 1
                self.last_check = max(self.last_check, event_time)
                
                if license_plate and license_plate != 'UNREAD':
                    logger.info(f"✓ Plate: {license_plate} ({confidence}%) | {self.lpr_cameras[event.camera_id]}")
                else:
