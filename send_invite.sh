...
HOST=$(echo "$UNIFA_API_URL" | sed -E 's|https?://([^:/]+).*|\1|')
...
if [ -n "$USER_EMAIL" ]; then
    echo "Verifying User ID for $USER_EMAIL on Site 1..."
...
echo "Sending UniFi Identity Invitation to User ID: $USER_ID (Email: ${USER_EMAIL:-N/A}) on Site 1..."
